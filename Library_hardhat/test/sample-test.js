...
describe("Library contract", function () {
    ...
    beforeEach(async function () {
      ...
      library = await Library.deploy(200);
    ...
    describe("AddBook tests", function () {
      it("Should add a book and emit an event with its id and the book itself", async function () {
        ...
        expect(receipt.events[0].args.id).to.equal(78);
        expect(receipt.events[0].args.book.name).to.equal("foo");
      ...
      it("Should add multiple books", async function () {
        ...
        // }
        // expect(receipt.events[1].args.id).to.equal(78);
        // expect(receipt.events[1].args.book.name).to.equal("foo");
        ...
        // expect(receipt.events[1].args.id).to.equal(78);
        // expect(receipt.events[1].args.book.name).to.equal("bar");
        // expect(receipt.events[1].args.book.availableCopies).to.equal(1);
        // expect(receipt.events[1].args.book.previouslyTaken).to.deep.equal([]);

        // expect(receipt.events[3].args.id).to.equal(78);
        // expect(receipt.events[3].args.book.name).to.equal("bar");
    ...
    describe("takeBook tests", function () {
      it("Should take a book and pay a fee", async function () {
        ...
        const tx = await library.takeBook(78, {value: ethers.utils.parseEther("0.00000000000000020")});
        const receipt = await tx.wait();
        expect(receipt.events[0].args.id).to.equal(78);
        expect(receipt.events[0].args.book.name).to.equal("foo");
      ...
      it("Should take a book and pay a fee and return the extra eth", async function () {
        await library.addBook("foo");
        const tx = await library.takeBook(78, {value: ethers.utils.parseEther("0.001")});
        const receipt = await tx.wait();
        expect(receipt.events[0].args.id).to.equal(78);
        expect(receipt.events[0].args.book.name).to.equal("foo");
        expect(receipt.events[0].args.book.availableCopies).to.equal(0);
      ...
      it("Should throw error for value < fee", async function () {
        await library.addBook("foo");
        await expect(library.takeBook(78, {value: ethers.utils.parseEther("0.00000000000000019")}))
      ...
      it("Should throw error for user that already took this book ", async function () {
        await library.addBook("foo");
        await library.addBook("foo");
        await library.takeBook(78, {value: ethers.utils.parseEther("0.0000000000000002")});
        await expect(library.takeBook(78, {value: ethers.utils.parseEther("0.0000000000000002")}))
      ...
      it("Should throw error for unavailable book(already taken)", async function () {
        await library.addBook("foo");
        await library.takeBook(78, {value: ethers.utils.parseEther("0.0000000000000002")});
        await expect(library.connect(addr1).takeBook(78, {value: ethers.utils.parseEther("0.0000000000000002")}))
    ...
    describe("returnBook tests", function () {
      it("Should return a book", async function () {
        await library.addBook("foo");
        await library.takeBook(78, {value: ethers.utils.parseEther("0.001")});
        const tx = await library.returnBook(78);
        const receipt = await tx.wait();
        expect(receipt.events[0].args.id).to.equal(78);
        expect(receipt.events[0].args.book.name).to.equal("foo");
        expect(receipt.events[0].args.book.availableCopies).to.equal(1);
      ...
      it("Should throw an error for a book the user has not taken", async function () {
        await library.addBook("foo");
        await expect(library.returnBook(78))
    ...
    describe("listUsersWhoTookBook tests", function () {
      it("Should return an array with all users that took acertain book", async function () {
        await library.addBook("foo");
        await library.takeBook(78, {value: ethers.utils.parseEther("0.001")});
        await library.returnBook(78);
        expect(await library.listUsersWhoTookBook(78)).to.deep.equal([owner.address]);
        await library.connect(addr1).takeBook(78, {value: ethers.utils.parseEther("0.001")});
        await library.connect(addr1).returnBook(78);
        expect(await library.listUsersWhoTookBook(78)).to.deep.equal([owner.address, addr1.address]);
      });
      it("Should throw error for nonexistent book", async function () {
        await expect(library.listUsersWhoTookBook(78))
        .to.be.revertedWith("This book is not available");
      });
    });
    ...
    describe("getAvailableBooks tests", function () {
      it("Should return an array with the ids of all available books", async function () {
        await library.addBook("foo");
        expect(await library.getAvailableBooks()).to.deep.equal([78]);
        await library.connect(addr1).takeBook(78, {value: ethers.utils.parseEther("0.001")});
        await expect(library.getAvailableBooks())
        .to.be.revertedWith("There are no available books");
        await library.connect(addr1).returnBook(78);
        expect(await library.getAvailableBooks()).to.deep.equal([78]);
        await library.addBook("foo");
        await library.connect(addr1).takeBook(78, {value: ethers.utils.parseEther("0.001")});
        ...
        await library.returnBook(78);
        expect(await library.getAvailableBooks()).to.deep.equal([78, 81]);

      });
      it("Should throw error for no available books", async function () {
        await expect(library.getAvailableBooks())
        .to.be.revertedWith("There are no available books")
      });
    });
